...
export function getPaisesByName(name) {
  ...
  return paises.find(pais => pais?.pais?.toLowerCase() === name.toLowerCase());
}
// Agregar un nuevo país
export function addPais(newPais) {
  if (!newPais?.pais) throw new Error('El objeto debe tener la propiedad "pais"');
  const paises = getPaises();
  paises.push(newPais);
...
export function deletePais(name) {
  ...
  paises = paises.filter(pais => pais?.pais && pais.pais.toLowerCase() !== name.toLowerCase());
